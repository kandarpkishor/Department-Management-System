...
<html>
	...
	<div class="main">
			...
			<div class="temp_cont">
				...
				<div class="temp" style="border:2px solid red;"> <a href="attendance_report.php"><img src="image/attendance.jpg" height="100%" width="100%"/></a></div>
			...
			<div class="right_panel">
				<div class="item">
					...
					<?php
						...
						$year=date('Y');
						$sl_no=0;
						$query2 = mysqli_query($conn,"SELECT *FROM attendance WHERE MONTH(DATE)='$month' AND YEAR(DATE)='$year' AND S_ID='$login_user_roll'");
						//$row = mysqli_fetch_array($query1,MYSQLI_BOTH);
						echo '<table style="margin:auto;" class="table">';
						echo '<th></th><th>DATE</th><th>ATTENDANCE</th><th>REMARK</th>';
						while($row = mysqli_fetch_array($query2,MYSQLI_BOTH))
						{
							$sl_no=$sl_no+1;
							$enc= new Encryption;
							$encdata=$enc->safe_b64encode($row['S_ID']);
							echo '<tr><td style="text-align:center;">'.$sl_no.'</td><td style="text-align:center;">'.$row['DATE'].'</td><td style="text-align:center;">'.$row['ATTENDANCE'].'</td><td style="text-align:center;">'.$row['LEAV_REMARK'].'</td>';
						}
						echo '</table>';
