...
<html>
	...
	<div class="main">
			...
			<div class="temp_cont">
				...
				<div class="temp" style="border:2px solid red;"><a href="assignment.php"><img src="image/assignment.jpg" height="100%" width="100%"/></a></div>
			...
			<div class="right_panel">
				<div class="item">
					...
					<?php
						...
						$year=date('Y');
						$sl_no=0;
						$query2 = mysqli_query($conn,"SELECT *FROM assignment");
						//$row = mysqli_fetch_array($query1,MYSQLI_BOTH);
						echo '<table style="margin:auto;" class="table">';
						echo '<th></th><th>DATE</th><th>ASSIGNMENT TOPIC</th><th>STATUS</th>';
						while($row = mysqli_fetch_array($query2,MYSQLI_BOTH))
						{
							$sl_no=$sl_no+1;
							$enc= new Encryption;
							$encdata=$enc->safe_b64encode($row['S_ID']);
							echo '<tr><td style="text-align:center;">'.$sl_no.'</td><td style="text-align:center;">'.$row['DATE'].'</td><td style="text-align:center;">'.$row['ASSIGNMENT'].'</td><td style="text-align:center;">'.strtoupper($row['STATUS']).'</td>';
						}
						echo '</table>';
