				...
				<div class="item">
					...
					<form>
						<select name="subject">
							...
							<?php
							$query_sub=mysqli_query($conn,"SELECT SUBJECT FROM subject WHERE SEM='$login_user_sem'");
							$num=mysqli_num_rows($query_sub);
							while($row=mysqli_fetch_array($query_sub,MYSQLI_BOTH))
							{
								?>
					...
					<h3>Unit Covered till date</h3>
					UNIT-I
<p>Set Theory: Definition of Sets, Venn Diagrams, complements, Cartesian products, power
sets, countingprinciple, cardinality and countability (Countable and Uncountable sets),
proofs of some generalidentities on sets, pigeonhole principle</p>
				</div>
			</div>
	</div>
	</body>
